# app/api/v1/institutions_data.py (abacadaba)
...
async def get_institution_admissions(
    ...
    academic_year: Optional[str] = Query(
        None, description="Filter by academic year (e.g., '2023-24')"
    ),
    ...
    if not admissions:
        raise HTTPException(
            status_code=404, detail="No admissions data found for this institution"
        )
...
async def get_institution_tuition(
    ...
    if not tuition:
        raise HTTPException(
            status_code=404, detail="No tuition data found for this institution"
        )
...
async def get_financial_overview(ipeds_id: int, db: AsyncSession = Depends(get_db)):
    """Get a combined overview of the most recent tuition and admissions data"""
    # Get most recent tuition data
    tuition_query = (
        select(TuitionData)
        .where(TuitionData.ipeds_id == ipeds_id)
    ...
    admissions_query = (
        select(AdmissionData)
        .where(AdmissionData.ipeds_id == ipeds_id)
    ...
    if not tuition and not admissions:
        raise HTTPException(
            status_code=404, detail="No financial or admissions data found"
        )
    ...
    return {
        ...
        "admissions": (
            AdmissionDataResponse.from_orm(admissions) if admissions else None
        ),
    }
