...
async def validate_invitation(
    ...
    if not invitation:
        return InvitationValidateResponse(
            valid=False, message="Invalid or already used invitation code"
        )

    # Check expiration (handle timezone-naive datetime from database)
    if invitation.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
        invitation.status = InvitationStatus.EXPIRED
        await db.commit()
        return InvitationValidateResponse(
            valid=False, message="This invitation code has expired"
        )
...
@limiter.limit(RATE_LIMITS["auth"])
async def register(
    request: Request, admin_data: AdminRegister, db: AsyncSession = Depends(get_db)
):
    ...
    db.add(new_admin)
    await db.flush()  # Flush to get the new_admin.id

    # Mark invitation as claimed
    invitation.status = InvitationStatus.CLAIMED
    invitation.claimed_by = new_admin.id
    invitation.claimed_at = datetime.utcnow()  # Use timezone-naive
